...
type Props = {
    setAddressTrigger: (trigger: boolean) => void;
  AdrsTrigger: boolean;
};
...
const TimeSlotSteps = ({ AdrsTrigger,setAddressTrigger }: Props) => {
  ...
  const GetSingleLoc = async (loc?: string) => {
    try {
      ...
      const res = await axios.get(
        `/api/manage/get-singlelocation/${location}`
      );
  ...
  const onChangeDate = (
    ...
    if (month && year) {
      const result = Mslots?.monthlyslots.find(
        (slot: { month: string; year: number }) =>
          slot.month === month && slot.year === year
      );
      setSelectedMonth(result?.slots || null);

      for (let t of Mslots?.monthlyslots||null) {
      ...
      <div className="p-4 sm:block md:hidden ">
        <div className="p-1 flex justify-between">
          ...
          <DatePicker
            picker="month"
            size="large"
            className="border border-gray-500 h-fit rounded-[0.300rem]"
        ...
        {SelectedMonth ? (
          <>
            <div
              className="flex space-x-4 p-2 overflow-x-scroll CBhide-scrollbar"
              ref={scrollRef}
            >
              {SelectedMonth?.map((day, index) => {
                const availableTimeCount = day.time?.filter(
                  (time) => time.available
                ).length;
                return (
                  <div
                    onClick={() => {
                      Selectdate(
                        day.dayname,
                        day.day,
                        SelectedMonthDet?.month,
                        SelectedMonthDet?.year,
                        day.time
                      );
                    }}
                    key={index}
                  ...
                  >
                    ...
                    <h1 className="text-black text-2xl m-3 font-bold">
                      {day.day}
                    </h1>
                    <div className="flex flex-row justify-center items-center">
                      <h1
                        className={`${
                          availableTimeCount <= 2
                            ? "text-red-700"
                            : "text-green-700"
                        }  text-md font-extrabold flex mr-2`}
                      >
                        O
                      </h1>
            ...
            {/* Time */}
            <div
              className={SelectedMonth && date.dayNumber ? "block" : "hidden"}
            >
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-2 lg:grid-cols-4 p-2">
                {slots?.map((t) => (
                  <div
                    ...
                    className={`flex items-center text-black justify-center w-36 h-12 border border-gray-500 rounded-md flex-col ${
                      t.available
                        ...
                        : "hover:cursor-not-allowed bg-gray-200"
                    }`}
                  >
                    <h1
                      className={` text-xl ${
                        !t.available ? "text-gray-500" : ""
                      }`}
                    >
                      {t.time}
                    </h1>
          ...
          <div className="flex justify-center items-center">
            <p>
              Please select a month to continue or sometimes the slots for the
              selected month haven't been finalized yet.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="hidden md:block">
        ...
        {SelectedMonth ? (
          <>
            <div
              className={`${
                SelectedMonth ? "flex justify-end mb-4" : "hidden"
              }`}
            >
              <button
              ...
              <div className="flex space-x-5">
                {slots?.map((t) => (
                  <div
                    key={t._id}
                    onClick={() => t.available && SelectTime(t.time, t._id)}
                    className={`flex items-center text-black justify-center w-36 h-12 rounded-md border border-gray-300 shadow-lg flex-col ${
                      t.available
                        ? `hover:cursor-pointer ${
                            time === t.time
                              ? "bg-violet-600 text-white"
